...
from bc_engine import application
from bc_engine.collider import *
from bc_engine.mesh import Mesh
from bc_engine.sequence import Sequence, Func, Wait
from bc_engine.ursinamath import lerp
from bc_engine import curve
from bc_engine.curve import CubicBezier
from bc_engine import mesh_importer
from bc_engine.mesh_importer import load_model
from bc_engine.texture_importer import load_texture
from bc_engine.string_utilities import camel_to_snake
from textwrap import dedent
from panda3d.core import Shader as Panda3dShader
from bc_engine.shader import Shader
from bc_engine.string_utilities import print_info, print_warning

from bc_engine import color
from bc_engine.color import Color
try:
    from bc_engine.scene import instance as scene
except:
    pass
...
class Entity(NodePath):
    ...
    def combine(self, analyze=False, auto_destroy=True, ignore=[]):
        from bc_engine.scripts.combine import combine
...
#------------
    def animate(self, name, value, duration=.1, delay=0, curve=curve.in_expo, loop=False, resolution=None, interrupt='kill', time_step=None, auto_destroy=True):
        ...
        if delay:
            from bc_engine.ursinastuff import invoke
    ...
    def intersects(self, traverse_target=scene, ignore=(), debug=False):
        ...
        from bc_engine.hit_info import HitInfo

        if not self.collision or not self.collider:
            self.hit = HitInfo(hit=False)
            return self.hit

        from bc_engine import distance
